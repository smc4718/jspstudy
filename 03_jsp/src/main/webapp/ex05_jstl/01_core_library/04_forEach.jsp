...
<body>
  ...
  <%-- 
   ...
   <%-- 5 ~ 1 출력 --%>
   <c:forEach var="n" begin="1" end="1" step="1">  <%-- 증감이아니라 증가값이기에 "-"는 안됨 --%>
    <div>${6 - n}</div>
   </c:forEach>
   ...
   pageContext.setAttribute("seosons", session);
   %>
   <c:forEach var="seoson" items="${seasons}" varStatus="vs">
   ...
   <%
     ...
     List<BlogPost> list = Arrays.asList(
       ...
       new BlogPost(2, "두 블로그", 15, LocalData.of(2023, 1, 2)),
       new BlogPost(3, "세 블로그", 20, LocalData.of(2023, 1, 3))
     );
